...
usage() {
  cat <<USAGE
Usage: install.sh --project-dir PATH --app-name NAME --platform ios|ipados|macos|visionos|watchos [options]

Options:
  --sim-name NAME           Simulator name (default: auto for simulator platforms)
...
if [[ -z "$SIM_NAME" && "$PLATFORM" != "macos" ]]; then
  SIM_NAME="auto"
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TEMPLATE_ROOT="$SCRIPT_DIR/../assets/toolkit"
...
if [[ $DRY_RUN -eq 1 ]]; then
  echo "[dry-run] Would install Xcode build harness"
...
echo "Installed Xcode build harness to $PROJECT_DIR"
